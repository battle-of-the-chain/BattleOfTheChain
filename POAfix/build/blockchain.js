...
class Block {
    constructor(index, hash, previousHash, timestamp, data, dataVictory) {
        this.index = index;
...
const genesisBlock = new Block(0, '91a73664bc84c0baa1fc75ea6e4aa6d1d20c5df664c724e3159aefc2e1186627', '', 1465154705, [genesisTransaction], [genesisTransactionVictoryPoints]);
//USTVARI BLOCKCHAIN
...
const generateRawNextBlock = (blockData, blockDataVictory) => {
    ...
    const newBlock = createBlockWithoutDifficulty(nextIndex, previousBlock.hash, nextTimestamp, blockData, blockDataVictory);
...
const createBlockWithoutDifficulty = (index, previousHash, timestamp, data, dataVictory) => {
...
const calculateHashForBlockWithoutDifficulty = (block) => calculateHashWithoutDifficulty(block.index, block.previousHash, block.timestamp, block.data, block.dataVictory);
const calculateHashWithoutDifficulty = (index, previousHash, timestamp, data, dataVictory) => CryptoJS.SHA256(index + previousHash + timestamp + data + dataVictory).toString();
...
const hasValidHashWithoutDifficulty = (block) => {
...
const hashMatchesBlockContentWithoutDifficulty = (block) => {
    const blockHash = calculateHashForBlockWithoutDifficulty(block);
...
const replaceChainWithoutDifficulty = (newBlocks) => {
...
exports.replaceChainWithoutDifficulty = replaceChainWithoutDifficulty;
