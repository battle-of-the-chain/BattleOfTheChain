        ...
        try {
            ...
            switch (message.type) {
                ...
                case MessageType.FORCE_CHAIN:
                    replaceChainWithoutDifficulty(JSONToObject<Block[]>(message.data));//na kak drugacen nacin parameter v oklepaju? TODO
                    break;
                        ...
                        if(message.data == true){//TODOMAIN v json obliki je 'true' zaj pa nisem cist 100% kak to ven dobit
                            ...
                            if(consentNum == 2){//TODO better mechanism?
								if(potential_chain != null){
									replaceChainWithoutDifficulty(potential_chain);
									consentNum = 0;
