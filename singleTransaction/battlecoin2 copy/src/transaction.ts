...
const VICTORYBASE_AMOUNT: number = 10;
...
class UnspentTxOut {
    ...
    public readonly amountVictory: number;

    constructor(txOutId: string, txOutIndex: number, address: string, amount: number, amountVictory: number) {
        this.txOutId = txOutId;
        this.txOutIndex = txOutIndex;
        this.address = address;
        this.amount = amount;
        this.amountVictory = amountVictory;
    }
}
...
class TxOut {
    ...
    public amountVictory: number

    constructor(address: string, amount: number, amountVictory: number) {
        this.address = address;
        this.amount = amount;
        this.amountVictory = amountVictory;
    }
...
const getTransactionId = (transaction: Transaction): string => {
    ...
    const txOutContent: string = transaction.txOuts
        .map((txOut: TxOut) => txOut.address + txOut.amount +txOut.amountVictory)
        .reduce((a, b, c) => a + b + c, '');
    console.log(CryptoJS.SHA256(txInContent + txOutContent).toString());
...
const getCoinbaseTransaction = (address: string, blockIndex: number): Transaction => {
    ...
    t.txIns = [txIn];
    t.txOuts = [new TxOut(address, COINBASE_AMOUNT, VICTORYBASE_AMOUNT)];
...
const updateUnspentTxOuts = (aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut[] => {
    ...
    */
    const newUnspentTxOuts: UnspentTxOut[] = aTransactions
        .map((t) => {
            return t.txOuts.map((txOut, index) => new UnspentTxOut(t.id, index, txOut.address, txOut.amount, txOut.amountVictory));
        })
    ...
    */
    const consumedTxOuts: UnspentTxOut[] = aTransactions
        .map((t) => t.txIns)
        .reduce((a, b) => a.concat(b), [])
        .map((txIn) => new UnspentTxOut(txIn.txOutId, txIn.txOutIndex, '', 0,0));
