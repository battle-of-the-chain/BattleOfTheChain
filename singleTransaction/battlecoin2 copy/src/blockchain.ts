...
const genesisTransaction = {
    ...
    'id': '9ba0a8ac4717a069248b6f401b989cfebfd7e09cdfa96780f60db6c78e4bb118'//'e655f6a5f26dc9b4cac6e46f52336428287759cf81ef5ff10854f69d68f43fa3'
    //Če se spreminja vrednosti v tej transakciji, se mora spremeniti tudi id
};
...
const generatenextBlockWithTransaction = (receiverAddress: string, amount: number, amountVictory: number) => {
    ...
    const coinbaseTx: Transaction = getCoinbaseTransaction(getPublicFromWallet(), getLatestBlock().index + 1);
    const tx: Transaction = createTransaction(receiverAddress, amount, amountVictory, getPrivateFromWallet(), getUnspentTxOuts(), getTransactionPool());
...
const sendTransaction = (address: string, amount: number, amountVictory: number): Transaction => {
    const tx: Transaction = createTransaction(address, amount, amountVictory, getPrivateFromWallet(), getUnspentTxOuts(), getTransactionPool());
