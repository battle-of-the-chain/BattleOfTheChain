...
const createTxOuts = (receiverAddress: string, myAddress: string, amount,amountVictory, leftOverAmount: number) => {
    const txOut1: TxOut = new TxOut(receiverAddress, amount, amountVictory);
    if (leftOverAmount === 0) {
        return [txOut1];
    } else {
        const leftOverTx = new TxOut(myAddress, leftOverAmount, amountVictory); //SPREMENI V LEFTOVER VICTORY
...
const createTransaction = (receiverAddress: string, amount: number, amountVictory:number, privateKey: string,
    ...
    tx.txOuts = createTxOuts(receiverAddress, myAddress, amount, amountVictory, leftOverAmount);
