...
import {
	...
	getAccountBalance,getAccountBalanceVictoryPoints
} from './blockchain';
...
import {changeKeyToOperateWith, resetKey} from './wallet'
            ...
            switch (message.type) {
					...
					case MessageType.BLOCKS:
						...
						if(p_key != null){
							changeKeyToOperateWith(p_key);
							try{
								var blockChainToAnalyse = JSON.stringify(getBlockchain(), null, 2);
								if (blockChainToAnalyse === null) {
									//res.status(400).send('could not generate block');
									console.log('Could not fetch blockChain');
								} 
								else {
									var numberOfBlocks = blockChainToAnalyse.length;
									if(numberOfBlocks != null){
										console.log("There are " + numberOfBlocks + " blocks in the chain");
									}
									else{
										console.log("Blockchain is null!");
									}
									//console.log("Balance (victory points) of "+ p_key + " is " + balance);
								}
							}
