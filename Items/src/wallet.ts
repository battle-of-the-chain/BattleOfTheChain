...
/*
      BALANCE ZA CURRENCY
*/
const getBalance = (address: string, unspentTxOuts: UnspentTxOut[], currency: string): number => {
...
*/
const findUnspentTxOuts = (ownerAddress: string, unspentTxOuts: UnspentTxOut[], currency: string) => {
    return _.filter(unspentTxOuts, (uTxO: UnspentTxOut) => (uTxO.address === ownerAddress && uTxO.currency === currency));
};
...
*/
const createTxOuts = (receiverAddress: string, myAddress: string, amount, leftOverAmount: number, currency: string) => {
    const txOut1: TxOut = new TxOut(receiverAddress, amount, currency);
    if (leftOverAmount === 0) {
        return [txOut1];
    } else {
        //maybe
        const leftOverTx = new TxOut(myAddress, leftOverAmount, currency);
