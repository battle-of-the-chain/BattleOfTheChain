...
class Block {
    ...
    constructor(index: number, hash: string, previousHash: string,
                timestamp: number, data: Transaction[], /*ataVictory: TransactionVictoryPoints[],*/ difficulty: number, nonce: number) {
        this.index = index;
...
const getMyUnspentTransactionOutputs = () => {
    return findUnspentTxOuts(getPublicFromWallet(), getUnspentTxOuts(), "Eyy");
};
...
const generatenextBlockWithTransaction = (receiverAddress: string, amount: number) => {
    ...
    const coinbaseTx: Transaction = getCoinbaseTransaction(getPublicFromWallet(), getLatestBlock().index + 1, "Ayy");
...
const sendTransaction = (address: string, amount: number, currency: string): Transaction => {
    var unspent: UnspentTxOut[] = getUnspentTxOuts().filter((uTxO: UnspentTxOut): boolean => { return(uTxO.currency===currency)});
    console.log("___________________\n\n"+unspent+"___________________\n\n"+getUnspentTxOuts()+"___________________\n\n");
    const tx: Transaction = createTransaction(address, amount, getPrivateFromWallet(), unspent/*getUnspentTxOuts()*/, getTransactionPool());
    unspent = getUnspentTxOuts().filter((uTxO: UnspentTxOut): boolean => { return(uTxO.currency===currency)});
    addToTransactionPool(tx, unspent/*getUnspentTxOuts()*/);
