...
class UnspentTxOut {
    ...
    constructor(txOutId: string, txOutIndex: number, address: string, amount: number, currency: string) {
...
class TxOut {
    ...
    constructor(address: string, amount: number, currency: string) {
        this.address = address;
        this.amount = amount;
        this.currency = currency;
    }
...
const getCoinbaseTransaction = (address: string, blockIndex: number, currency: string): Transaction => {
    ...
    t.txIns = [txIn];
    //maybe
    //probaj prvo nardit attay, potem pa tega naprej dati
    t.txOuts = [new TxOut(address, COINBASE_AMOUNT, currency)];
    //t.txOuts[0].currency=currency;
...
const updateUnspentTxOuts = (aTransactions: Transaction[], aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut[] => {
    ...
    */
    const newUnspentTxOuts: UnspentTxOut[] = aTransactions
        .map((t) => {
            return t.txOuts.map((txOut, index) => new UnspentTxOut(t.id, index, txOut.address, txOut.amount, txOut.currency));
        })
    ...
    */
    ...
    const consumedTxOuts: UnspentTxOut[] = aTransactions
        .map((t) => t.txIns)
        .reduce((a, b) => a.concat(b), [])
        //tukaj ugotovi kako dobit notri txOut.currency Ayy
        .map((txIn) => new UnspentTxOut(txIn.txOutId, txIn.txOutIndex, '', 0, txIn.txOutCurr));
